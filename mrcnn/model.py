...
def resnet_graph(input_image, architecture, stage5=False, train_bn=True):
    ...
    # Stage 1 after stage 1, output shape is 1/4 input
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    Returns: 
        detections shaped: [num_detections, (y1, x1, y2, x2, class_id, score)] where
    ...
    # print ('probs', probs)
    indices = tf.stack([tf.range(config.POST_NMS_ROIS_INFERENCE), class_ids], axis=1)
...
def focal_loss(prediction_tensor, target_tensor, weights=None, alpha=0.25, gamma=2, sigmoided=False):
    ...
    """
    sigmoid_p = tf.sigmoid(prediction_tensor) if not sigmoided else prediction_tensor
...
def mrcnn_class_loss_graph(target_class_ids, pred_class_logits,
    ...
    loss = focal_loss(prediction_tensor=pred_class_logits, 
                      target_tensor=tf.cast(tf.one_hot(target_class_ids, tf.cast(active_class_ids[1][0], "int32")), "float32"))
...
def mrcnn_mask_loss_graph(target_masks, target_class_ids, pred_masks):
    ...
    # y_true_ = tf.cast(tf.one_hot(tf.cast(y_true, "int32"), 8), 'float32')
    # loss = K.switch(tf.size(y_true) > 0,
    #                 focal_loss(y_pred, y_true_, sigmoided=True),
    #                 tf.constant(0.0))
    # ----------------------------------------------------------
...
def data_generator(dataset, config, shuffle=True, augmentation=None,
    ...
    of the lists differs depending on the received arguments:
        inputs list:
            - images:       [batch, H, W, C]1408,
            - image_meta:   [batch, (meta data)] Image details1408,. See compose_image_meta()
            - rpn_match:    [batch, N] Integer (1=positive anc1408,hor, -1=negative, 0=neutral)
    ...
    while True:
        try:
            ...
            # Skip images that have no insta1408,pen in cases
            # where we train on a subset of 1408,age doesn't
            # have any of the classes we car1408,
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        # Pyramid Attention Network + FPN--------------------------------------------
        # P4 = KL.Add(name="fpn_p4add")([
        #     KL.UpSampling2D(size=(2, 2), name="fpn_p5upsampled")(P5),
        #     FPAModule(256, training=config.TRAIN_BN)(C4)])
        # P3 = KL.Add(name="fpn_p3add")([
        #     KL.UpSampling2D(size=(2, 2), name="fpn_p4upsampled")(P4),
        #     GAUModule(256, training=config.TRAIN_BN)(P4, C3)])
        # P2 = KL.Add(name="fpn_p2add")([
        #     KL.UpSampling2D(size=(2, 2), name="fpn_p3upsampled")(P3),
        #     GAUModule(256, training=config.TRAIN_BN)(P3, C2)])
        # ---------------------------------------------------------------------------
    ...
    def run_graph(self, images, outputs, image_metas=None):
        ...
        Returns: 
            an ordered dict of results. Keys are the names received in the
...
def norm_boxes_graph(boxes, shape):
    ...
    Note: 
        In pixel coordinates (y2, x2) is outside the box. But in normalized
